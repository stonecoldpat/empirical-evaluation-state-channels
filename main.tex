...
One player selects a slot to shoot using $\battleshipattackslot$ and the counterparty must reveal it using either $\battleshiprevealslot$ or $\battleshipsinking$ depending on whether this shot has revealed the final location for a ship (i.e. the opponent has sunk a ship and it must be declared). 
At the end, the winner must reveal their board and every ship's location to the loser using $\battleshiprevealboard$.
The loser is provided a fixed time period to prove if the winner's board was incorrectly set up using $\battleshipfraud$. 
...
\paragraph{Game Phases} There is $\gamestatus := \{\gamesetup, \gameattack, \gamereveal, \gamewinner,$ $ \gamefinished\}$. 
...
Next, each party computes $\hready_{i} := \hash(\hboard, \hship_{1},...,\hship_{k})$ where $k$ is the number of ships for this board\footnote{We ignore the type of ships that constitutes a valid board in this description as this depends on the (fun) game logic.}. 
...
If this shooting has not sank a ship, then the counter-party responds using $\battleshiprevealslot$ and the signed message $\sign_{\participant}(b, r, i, j, \instantiatedno, \appcontract)$.
...
The game transitions to $\gamefraud$ if the maximum number of possible moves has been exceedemor it transitions to $\gamewinner$ if one player has declared all their ships sunk. 
...
In the following, we define four different fraud proofs that can be submitted to the contract which demonstrate that a revealed board or its corresponding list of ships is not well-formed. 

\paragraph{Pre-images for slots or ship locations are not well-formed} 
The contract will not accept the slots or ship locations using $\battleshiprevealboard$.
This there is no need for a fraud proof as the player can never win the game. 

\paragraph{Ships not placed on the board}
This proof assumes the full board and the ship locations are revealed. 
The contract can verify that the pre-image of $\hslot$ indicates a marked position and it corresponds to the pre-image of a ship location $\hshiplocation$. 
If a ship location claims to be located at $i,j$, but the location is not marked in $\hslot$, then it is easily verified as fraud.
...
The contract can verify that the ships are adjacent if  $i,j$ is within the range $i'\pm1, j'\pm1$ and of course that both ships are claimed to be in the same location. 

\paragraph{Ships not placed horizontally or vertically} 
The contract is provided with all pre-images for the entire ship $\hshiplocation, \hshiplocation$ and it can verify that either every location has the same $i$ or $j$ co-ordinate. 
...
It is possible for the player to prove during (or at the end) of the game that the counterparty did not declare that their ship was sunk. 
As we mentioned in Section \ref{fig:statechannel}, if a ship was hit, but not declared as sunk, then the counterparty signs the following message: 
...
First, the player can finish the game early if their shots reveals two slots which are marked as water before and after a ship which has not been declared as sunk.
...
The contract can simply verify that the two slots are indeed before and after the ship, and that the counterparty did not declare the ship as sunk after any shots.
Second, the two additional slots are not required when $\gamestatus := \gamewinner$ as the full board (and list of ships) are revealed. 
The player can simply provide the list of signed reveal messages from the counterparty and identify which ship should have been sunk. 
The contract can verify that the counterparty did not declare the ship as sunk, but the ship was indeed fully hit. 
