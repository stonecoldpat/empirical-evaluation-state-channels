...
All parties may commit themselves to executing the application between each other and afterwards one party turns off the channel using the dispute process.  
This is problematic as the counterparty agreed to execute the application with an expectation that it will be within the state channel, but they are now committed to executing the entire application via the blockchain. 
...
This is accomplished as either party can update the state channel contract with the latest state and the dispute process is used to receive a list of commands from each party.
After the dispute process has expired, the state transition is performed on-chain as each command is executed in the application contract and the new state can then be used in the state channel. 
...
The approach of Perun and Counterfactual is to minimise on-chain state (i.e. a single multi-signature contract) and allow two parties to create/destory multiple applications within a state channel. 
On the other hand, Funfair's implementation of state channels aims to maximimise on-chain state for a single application. 
Both approaches have an implicit assumption about the likelihood that one party may trigger a dispute and the financial cost of resolving this dispute by sending the full state to the blockchain.
Fundamentally, we highlight that state channels introduce new economic attack vectors that must be consideration when designing compatible applications.
In fact, this dilemma can be summed up in a single question, i.e. \textit{does it make sense to resolve a dispute for a \$10 bet if it costs \$100 to submit the full state?} 
Our empirical evaluation confirms that based on the current state of the Ethereum network, the Funfair approach is preferable to avoid violating the safety guarantees provided by state channels. 
...
The accountable thrid party only requires a signature from every party in the channel $\Sigma_{\participant}$, the state hash $\hstate$ and the monotonic counter $\monotoniccounter$ to resolve disputes on the customer's behalf. 
