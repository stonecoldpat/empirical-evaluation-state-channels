...
%	}
	...
	\begin{abstract}
		 ...
		 Typically it is considered unreasonable to play via the blockchain which we confirm as a single game costs between \$16.27 and \$24.06, but it is perceived as an ideal application for a state channel. 
...
Since 2009, we have witnessed the rise of cryptocurrencies as the market capitalisation for all cryptocurrencies peaked at \$1 trillion US dollars in December 2017.
While Bitcoin \cite{nakamoto2008bitcoin} was the first cryptocurrency designed to support financial transactions, another prominent cryptocurrency called Ethereum \cite{wood2014ethereum} has emerged for executing programs called smart contracts. 
...
An application like battleship is not typically considered viable to execute via the blockchain due to the quantity of transactions required and in our experiment we confirm this perception as the financial cost is between \$16.27 and \$24.06. 
...
Of course, if there is no activity within the state channel, then the state channel contract's dispute process can expire without a submitted $\hstatei$.
...
Deploying both the modified and unmodified battleship contract highlights the cost for modifying an application contract to support a state channel is approximately 3 million gas. 
...
This worst-case costs \$24.06 (approx 30 million gas) to finish the game.  
Locking the battleship game, creating the state channel, performing the dispute process costs and unlocking the battleship game costs \$1.56 (approx 2 million gas). 
...
%In the case of battleship, both parties commit to the battleship game using $\battleshipbegin$ and afterwards must play the entire turn-based game via the blockchain which costs between \$16.27 and \$24.06. 
%
...
Both players must finish the game which costs approximately on average \$16.27 to \$24.06 and the latency incurred by playing the game via the blockchain requires a short challenge period (i.e. 5 minutes) to permit each transaction to be accepted into the blockchain. 
...
%%Battleship was chosen as it is considered unreasonable to play on the Ethereum blockchain (i.e. \$16.27-\$24.06 per game), but it is perceived as an ideal application for a state channel. 
...
\begin{table}
	...
	\begin{tabular}[]{l l c c}
		...
		\hline
		1 & Create BattleshipCon & 10,020,170 & 7.97 \\
		...
		\hline
		17 & Create Modified BattleshipCon & 13,607,0695 & 10.83 \\
		...
		\multicolumn{2}{l}{Worst case for game} & 30,237,372 & 24.06 \\
		\hline 
	\end{tabular}
