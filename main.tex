...
%This requires the state channel contract to be instantiated on the blockchain in advance and to hold the funds of all parties.  
%If there is an issue in a single application, then the dispute process can be used to instantiate the application, transfer coins to it and permit its execution to complete via the blockchain. 
...
%While we have left it as future work, our state channel construction is application-agnostic by design and can be adopted to support instantiating an application after the dispute process has concluded. 
%Although our empirical evaluation highlights that deploying the application as part of the dispute process may significantly increase the cost (i.e. an extra \$10.36 for battleship) and it is advisable to reduce this cost by instantiating smart contracts using existing code on the blockchain. 
...
%
%Patrick McCorry and Chris Buckland are supported by an Ethereum Foundation scaling grant. We thank the IC3-ETH participants Frank Sauer, Matthew Salazar, Oded Naor, and Deepak Maram for their support at kick-starting this project (and winning third prize). Finally we thank Master Workshop: Off the Chain for bringing together state channel researchers which helped bootstrap this paper. 
