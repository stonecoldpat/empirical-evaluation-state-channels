...
Both players must finish the game play via the blockchain which costs approximately on average \$16.27 to \$24.05. 
The latency incurred by playing the game via the blockchain may require a short challenge period (i.e. 5 minutes) for each move to permit each transaction to be accepted into the blockchain.
As a result it may take several hours to complete a game and it is likely some players will forfeit their deposit (and bet) to simply quit the game early. 
...
The requirement to co-operate introduces new economic and time-based attacks that must be considered when designing the application. 
